...
class TaskManager(OdooBase):
    ...
    def move_subtask(self, subtask_id, new_parent_id, target_project_id=None):
        ...
        try:
            ...
            if not subtask_records:
                return {'success': False, 'error': f'Subtask with ID {subtask_id} not found'}
            ...
            if not parent_records:
                return {'success': False, 'error': f'Parent task with ID {new_parent_id} not found'}
            ...
            if target_project_id:
                ...
                if not project_records:
                    return {'success': False, 'error': f'Project with ID {target_project_id} not found'}
                project_name = project_records[0].name

            # Check for circular dependency
            if self._would_create_circular_dependency(subtask_id, new_parent_id):
                return {'success': False, 'error': 'Cannot move task: would create circular dependency'}
            ...
            if success:
                return {'success': True, 'subtask_name': subtask.name, 'new_parent_name': new_parent.name, 'project_name': project_name}
            else:
    ...
    def promote_task(self, task_id):
        ...
        try:
            ...
            if not hasattr(task, 'parent_id') or not task.parent_id:
                return {'success': False, 'error': 'Task is already a main task (no parent)'}
            ...
            if success:
                return {'success': True, 'task_name': task.name, 'former_parent_name': former_parent_name}
            else:
                return {'success': False, 'error': 'Write operation failed'}
    ...
    def move_multiple_subtasks(self, subtask_ids, new_parent_id, target_project_id=None):
        ...
        try:
            ...
            return {'success': moved_count > 0, 'moved_count': moved_count, 'failed_count': failed_count, 'errors': errors, 'new_parent_name': new_parent.name}
    ...
    def show_hierarchy(self, task_id, max_depth=3):
        ...
        try:
            ...
            # Build hierarchy
            hierarchy = {'main_task': self._task_to_dict(main_task), 'parents': self._get_parent_chain(task_id),
    ...
    def _get_children_recursive(self, task_id, max_depth, current_depth=0):
        ...
        try:
            if self.verbose:
                ...
                # First, let's debug what fields are available on the parent task
            if self.verbose:
                try:
                    parent_task = self.tasks.browse(task_id)
                    print(f"   Debug: Parent task fields: {dir(parent_task)}")

                    # Check for various child-related fields                                                                                                                                                                                  
                    child_fields = ['child_ids', 'subtask_ids', 'children', 'sub_task_ids']
                    for field in child_fields:
                        if hasattr(parent_task, field):
                            field_value = getattr(parent_task, field)
                            print(f"   Debug: Found field '{field}': {field_value} (type: {type(field_value)})")
                            if hasattr(field_value, '__len__'):
                                print(f"   Debug: Field '{field}' length: {len(field_value)}")
            ...
            try:
                ...
                if self.verbose:
                    ...
                    # Method 2: Try different parent field names
            if not child_records:
                parent_field_names = ['parent_task_id', 'parent', 'main_task_id', 'parent_task']
                for field_name in parent_field_names:
                    try:
                        search_domain = [(field_name, '=', task_id)]
                        test_records = self.tasks.search_records(search_domain)
                        if test_records:
                            child_records = test_records
                            if self.verbose:
                                print(f"   Method 2 ({field_name} =): Found {len(child_records)} children")
                            break
                    except Exception as e2:
                        if self.verbose:
                            print(f"   Method 2 ({field_name}) failed: {e2}")

                            # Method 3: Browse parent and check child fields
            if not child_records:
                try:
                    parent_task = self.tasks.browse(task_id)
                    ...
                    if self.verbose:
                        ...
                        # Method 4: Search all tasks and check which ones have this task as parent
            if not child_records and self.verbose:
                try:
                    print(f"   Method 4: Searching all tasks for parent references...")
                    # Get a sample of tasks to see their structure                                                                                                                                                                            
                    sample_tasks = self.tasks.search_records([], limit=5)
                    for sample in sample_tasks:
                        print(f"   Sample task {sample.id} fields: {[attr for attr in dir(sample) if not attr.startswith('_')]}")
                        # Check if any field contains our task_id                                                                                                                                                                             
                        for attr in dir(sample):
                            if not attr.startswith('_'):
                                try:
                                    value = getattr(sample, attr)
                                    if value == task_id or (hasattr(value, 'id') and value.id == task_id):
                                        print(f"   Found reference in field '{attr}': {value}")
                                except:
                                    pass
            ...
            for child in child_records:
                ...
                if current_depth + 1 < max_depth:
                    child_dict['children'] = self._get_children_recursive(child.id, max_depth, current_depth + 1)
    ...
    def _task_to_dict(self, task):
        ...
        task_dict = {'id': task.id, 'name': getattr(task, 'name', f'Task {task.id}'), }
